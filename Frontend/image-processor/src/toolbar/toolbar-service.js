...
import { getFileFromGraph, getFileSnakemake, resetGraphFromFile } from '../file/file';
import { App } from '../app';
export class ToolbarService {
    ...
    constructor(element, cmd, graph) {
        ...
        this.toolbar.on('file:pointerclick', (_evt) => {
            ...
            contextToolbar.on('action:save_txt',() => {
                contextToolbar.remove();
                const str = JSON.stringify(getFileFromGraph(graph));
                const bytes = new TextEncoder().encode(str);
                const blob = new Blob([bytes],{ type: 'application/json'});
                const el = window.document.createElement('a');
                el.href = window.URL.createObjectURL(blob);
                el.download = 'workflow.json';
                document.body.appendChild(el);
                el.click();
                document.body.removeChild(el);
            })
            contextToolbar.on('action:test',() => {
                contextToolbar.remove();
                const str = JSON.stringify(getFileSnakemake(graph));
                console.log(str);
            })
